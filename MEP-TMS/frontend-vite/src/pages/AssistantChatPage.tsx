        ...
        {messages.map((msg, index) => {
          ...
          return (
            <div key={index} style={{
              ...
              {/* Thought Drawer / Tool execution logs (Assistant only) */}
              {isAssistant && msg.thoughtLog && msg.thoughtLog.length > 0 && (
                <div style={{
                  marginLeft: 48,
                  width: '100%',
                  minWidth: '320px',
                  ...
                  <button
                    onClick={() => setExpandedThoughtIndex(expandedThoughtIndex === index ? null : index)}
                    style={{
                      width: '100%',
                      padding: '10px 14px',
                      background: 'rgba(255, 255, 255, 0.02)',
                  ...
                  >
                    <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <Database size={14} />
                      MCP Agent Executed {msg.thoughtLog.length} Tool{msg.thoughtLog.length > 1 ? 's' : ''}
                    </span>
                    {expandedThoughtIndex === index ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                  </button>

                  {expandedThoughtIndex === index && (
                    <div style={{
                      padding: 12,
                    ...
                    }} className="glass-recessed">
                      {msg.thoughtLog.map((log, lIdx) => (
                        <div key={lIdx} style={{
                          borderLeft: `2px solid ${log.status === 'success' ? '#10b981' : log.status === 'failed' ? '#ef4444' : '#0284c7'}`,
                          paddingLeft: 12,
                          display: 'flex',
                          flexDirection: 'column',
                          gap: 4
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <span style={{
                              ...
                              color: log.status === 'success' ? '#10b981' : log.status === 'failed' ? '#ef4444' : '#0284c7'
                            }}>
                              {log.status === 'success' && <CheckCircle size={10} />}
                              {log.status === 'failed' && <AlertCircle size={10} />}
                              {log.status === 'running' && <Loader2 className="animate-spin" size={10} />}
                              {log.status}
                            </span>
                          </div>
                          ...
                          <div style={{ color: 'var(--text-muted)', fontSize: 11 }}>
                            <strong>Args:</strong> {JSON.stringify(log.arguments)}
                          </div>
